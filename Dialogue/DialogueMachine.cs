...
public class DialogueMachine
{
    ...
    public void ConfigureRegisterSteps()
    {
        _stateMachine.Configure(DialogueState.Registration)
            .OnEntryAsync(async () => await _dialogue.SendRegistrationSequence(_registrationPoll.First()))
            .InternalTransitionAsync<string>(_setPersonDataTrigger, async (text, t) =>
            {
                _registrationPoll.Current() switch
                {
                    DialogueState.NameInput => _dialogue.Data.Customer.Name = text,
                    DialogueState.GenderInput => void,
                    DialogueState.AgeInput => _dialogue.Data.Customer.Age = int.Parse(text),
                    DialogueState.HeightInput => _dialogue.Data.Customer.Height = int.Parse(text),
                    DialogueState.WeightInput => _dialogue.Data.Customer.Weight = int.Parse(text),
                    DialogueState.EmailInput => _dialogue.Data.Customer.Email = text,
                    _ => void,
                };
            ...
            .InternalTransition(DialogueTrigger.MaleInput, () => _dialogue.Data.Customer.Gender = true)
            .InternalTransition(DialogueTrigger.FemaleInput, () => _dialogue.Data.Customer.Gender = false)
            .Permit(DialogueTrigger.BackToMenu, DialogueState.Menu);
    }
